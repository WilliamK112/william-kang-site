...
bullet(m,'Cloudflare vinext #2521: propagated resolved Next.js server externals into Nitro traceDeps while preserving and de-duplicating user configuration; added regression coverage across default packages, custom externals, development mode, and production builds.','Cloudflare vinext #2521:',9.05,True)
bullet(m,'Microsoft APM #1971: enforced least-privilege 0o700 permissions for deployed plugin executables and normalized deny-list identities to prevent bypasses through casing, .git suffixes, or alternate GitHub URL forms; added integration and mutation-regression tests.','Microsoft APM #1971:',9.05,True)
bullet(m,'NVIDIA NemoClaw #5676: hardened agents-list parsing against warning text and multiple JSON response schemas while preserving agent-ID filtering; validated the change with targeted tests, linting, CLI builds, and type checking.','NVIDIA NemoClaw #5676:',9.05,True)
bullet(m,'Meta DotSlash #16: added portable SHA-256 verification for downloaded release assets before extraction, resolving platform-specific asset metadata and supporting sha256sum with a shasum fallback across macOS, Windows, and Linux release targets.','Meta DotSlash #16:',9.05,True)
